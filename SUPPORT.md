...
Motion Preview 在本机读取和解码资源，不会将预览文件上传到服务器。提交问题前，请从截图、日志和示例资源中移除个人信息、文件路径或其他敏感数据。
...
Motion Preview reads and decodes resources locally and does not upload preview files to a server. Before submitting an issue, remove personal information, file paths, and other sensitive data from screenshots, logs, or sample files.
